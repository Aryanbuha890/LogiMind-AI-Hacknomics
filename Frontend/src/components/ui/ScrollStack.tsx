...
const ScrollStack = ({
  ...
  const measureLayout = useCallback(() => {
    ...
    // Reset card transforms temporarily so we can measure clean static positions
    ...
    // Measure card static tops relative to the document / scroll parent
    ...
    // Restore cached transforms
    cardsRef.current.forEach((card, i) => {
      const cache = cachedTransforms[i];
      if (cache) {
        card.style.transform = cache.transform;
        card.style.filter = cache.filter;
      }
    });
  ...
  useLayoutEffect(() => {
    ...
    // Repeat measurements to ensure stabilization
    const t1 = setTimeout(measureLayout, 100);
    const t2 = setTimeout(measureLayout, 500);
    const t3 = setTimeout(measureLayout, 1500);
    ...
    // Setup smooth scrolling physics
    setupLenis();
