...
type Msg = { role: "user" | "ai"; text: string; sources?: { title: string; src: string }[] };
...
const knowledge = [
  ...
  { t: "Konecranes RTG Maintenance Manual", s: "Maintenance Manuals", c: "#8B5CF6" },
];
...
function CopilotPage() {
  ...
  const send = (text?: string) => {
    ...
    setTimeout(() => {
      const reply: Msg = {
        ...
        sources: [
          { title: "Predictive Maintenance Model v3.2", src: "Maintenance Manuals" },
  ...
  return (
    <div className="flex h-screen flex-col">
      <AppTopBar title="AI Copilot" subtitle="Multi-agent reasoning · RAG · Citations" />
      <div className="grid flex-1 grid-cols-12 overflow-hidden">
        ...
        <main className="col-span-12 md:col-span-6 xl:col-span-7 flex flex-col bg-background">
          ...
          <div className="border-t border-border p-4">
            ...
            <div className="mt-2 text-[10px] text-muted-foreground">
              Responses grounded in your port's documents. Verify critical actions.
            </div>
          </div>
        </main>
        ...
        <aside className="hidden xl:flex xl:col-span-3 flex-col border-l border-border bg-card/40">
          ...
          <div className="flex-1 overflow-y-auto px-3 space-y-2">
            {knowledge.map((k) => (
              <div key={k.t} className="rounded-lg border border-border bg-background p-3">
                <div
